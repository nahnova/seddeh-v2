...
export async function POST(request: Request) {
  try {
    ...
    const { naam, email, boek, bericht, website } = body;
    ...
    await resend.emails.send({
      ...
      html: `
        <h2>Nieuwe inzage aanvraag</h2>
        <table style="border-collapse: collapse; width: 100%;">
          <tr>
            <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Boek</td>
            <td style="padding: 8px; border: 1px solid #ddd;">${escapeHtml(boek)}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding: 8px; border: 1px solid #ddd; white-space: pre-line;">${escapeHtml(bericht || "Geen toelichting")}</td>
          </tr>
        </table>
      `,
    });
    ...
    console.error("Failed to send inzage request email:", error);
    return NextResponse.json(
